...
void MqttSettingsService::onMqttConnect(bool sessionPresent) {
  SerialDebug.print(F("Connected to MQTT, "));
  if (sessionPresent) {
    SerialDebug.println(F("with persistent session"));
  } else {
    SerialDebug.println(F("without persistent session"));
  }
}

void MqttSettingsService::onMqttDisconnect(AsyncMqttClientDisconnectReason reason) {
  SerialDebug.print(F("Disconnected from MQTT reason: "));
  SerialDebug.println((uint8_t)reason);
...
void MqttSettingsService::onStationModeGotIP(WiFiEvent_t event, WiFiEventInfo_t info) {
  if (_state.enabled) {
    SerialDebug.println(F("WiFi connection dropped, starting MQTT client."));
    onConfigUpdated();
  }
}
...
void MqttSettingsService::onStationModeDisconnected(WiFiEvent_t event, WiFiEventInfo_t info) {
  if (_state.enabled) {
    SerialDebug.println(F("WiFi connection dropped, stopping MQTT client."));
    onConfigUpdated();
  }
}
#elif defined(ESP8266)
...
void MqttSettingsService::configureMqtt() {
  ...
  if (_state.enabled && WiFi.isConnected()) {
    SerialDebug.println(F("Connecting to MQTT..."));
